...
HANDLE PASCAL RAROpenArchiveEx(struct RAROpenArchiveDataEx *r)
{
  ...
  try
  {
    ...
    *AnsiArcName=0;
    ...
    if (!Data->Arc.OpenComix(ArcName,r->Fd,FMF_OPENSHARED))
    {
...
bool PASCAL RarRewind(HANDLE hArcData)
{
  DataSet *Data=(DataSet *)hArcData;

  rewind(Data->Arc.GetHandle());
  clearerr(Data->Arc.GetHandle());

  return Data->Arc.RawSeek(0, SEEK_SET);
}
...
int PASCAL ReadCurrentComicFile(HANDLE hArcData,int Operation){
  DataSet *Data=(DataSet *)hArcData;
  try{
     Data->Cmd.DllError=0;
  Data->Cmd.DllOpMode=Operation;
